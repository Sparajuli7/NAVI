...
- **Character backstory seeds partially addressed** — EXP-038 added BACKSTORY SEEDS rule 5 to both `characterGen.json` templates requesting specific opinion, funny anecdote, pet peeve, and sensory detail. This is the prompt-level change. The full `personality_details` schema from RESEARCH_ROUND4.md (7 fields with validation rules, fallback pools, and system prompt injection format) is not yet implemented in code. The current rule asks the LLM to include backstory in the `detailed` field; the full implementation would add a structured `personality_details` JSON field to the Character type.
...
- **Sensory grounding scorer misses non-English sensory language** — The `hasSensory` check in `liveConversationTest.ts` was expanded in EXP-036 with 20+ scenario-specific words (hiss, steam, incense, neon, etc.) but still only detects English keywords. Seoul scenario scored 0/5 sensory despite rich Korean atmospheric text. A future improvement would add Korean/Japanese/French sensory word patterns.
- **Extended conversation quality degradation** — EXP-040 found that sensory grounding drops to 0% and open loops collapse to 0% after turn 6-8 in a 12-turn conversation (gemma4:e2b). NEVER rules and target language remain 100%. The "lost in the middle" effect worsens as conversation history fills the context window. SESSION PACING (EXP-030) is validated as the correct mitigation. A mid-conversation system prompt refresh mechanism could help for sessions that exceed 10 exchanges.
- **Compact prompt tier not yet integrated into production** — EXP-039 validated that compact core rules (~460 tokens) score 23% higher than full rules on qwen2.5:1.5b (3.8 vs 3.1). The compact rules exist in `liveConversationTest.ts` but are not integrated into `contextController.ts` or `coreRules.json`. Target language regressed (85% -> 40%) and needs a stronger LANGUAGE line before production use.
- **Emotional override partially addressed** — EXP-037 added FRUSTRATION vs CONFUSION distinction to `coreRules.json`. Frustration about the language (still engaged) now triggers target-language-with-glosses support instead of full native language switch. Genuine shutdown signals still trigger full switch. Tested with Kathmandu scenario: target language went from 40% to 100%. However, other emotional states (grief, homesickness, anxiety unrelated to language) are not yet handled -- only language-related frustration has explicit override logic.
...
- **personality_details not in Character type** — The `Character` interface in `types/` has no `personality_details` field. Needs to be added alongside `characterGen.json` template update. See RESEARCH_ROUND4.md Area 1.
