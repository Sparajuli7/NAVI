...
### Known Feature Gaps
- ~~**CRITICAL: System prompt token budget blown — MUST layers consume 99.5% (3056/3072)**~~ — Fixed in EXP-096: removed 7 redundant sections from `coreRules.json` that were duplicating behavior already handled by ConversationDirector skills (`open_loop`, `sensory_anchor`, `session_pacing`), `worldEvents.json`, and `warmthLevels.json`. Trimmed few-shot examples from 11 to 3. Total savings: ~2,019 tokens. coreRules now ~2,539 tokens (down from ~4,558), fitting within 3,072 budget with ~533 tokens for warmth/mood/relationship/arc/learning layers.
- ~~**Ollama backend not restored on app restart**~~ — Fixed in EXP-095. Constructor only checked `'webllm'|'openrouter'` in `savedBackendPref`, missing `'ollama'`. Also Ollama model name was not persisted. Fix: added `'ollama'` to constructor check, persist `navi_ollama_model` in localStorage.
- **Token budget marginally tight post-EXP-096** — EXP-094 audit shows MUST+HIGH layers total ~3,422 tokens (350 over 3,072 budget). Core Rules may still be partially dropped. Consider raising budget to 4,096 for models with 8K+ context windows (gemma4, OpenRouter models).
