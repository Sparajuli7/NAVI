...
- **Character backstory seeds not generated** — `characterGen.json` produces personality descriptors but no specific opinions, anecdotes, preferences, or quirks. The warmth progression framework has nothing to progressively reveal. EXP-034 proved that specific personality details score 4.9/5.0 vs 3.1 for generic traits. **RESEARCH_ROUND4.md Area 1 provides exact replacement prompt templates** with `personality_details` schema (7 fields: strong_opinion, funny_anecdote, sensory_anchor, pet_peeve, recurring_character, favorite_spot, unpopular_take), validation rules, fallback pools per city, and system prompt injection format. Ready for implementation.
...
- **Sensory grounding scorer misses non-English sensory language** — The `hasSensory` check in `liveConversationTest.ts` only detects English keywords (smell, hear, rain, etc.). Models using Japanese, Korean, or French sensory vocabulary score 0 on sensory despite vivid grounding. Seoul scenario scored 0/5 sensory despite rich atmosphere because all sensory words were in Korean.
