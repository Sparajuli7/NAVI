...
### Resolved Feature Gaps (2026-04-16u — EXP-053 through EXP-055)
- ~~**CRITICAL: Memory stores global, not per-character (EXP-053)**~~ — Phrases learned with a Nepali companion showed up when talking to a French companion. `LearnerProfileStore` query methods (`getPhrasesForReview`, `getStrugglingPhrases`, `getUrgentReviewPhrases`, `getRoutineReviewPhrases`, `formatForPrompt`, `getCurrentStage`) now accept optional `language` parameter. All call sites pass `location.getPrimaryLanguage()`. UI components (`FlashcardDeck`, `KnowledgeGraphScreen`) receive language-filtered phrases. `postProcess` now records phrases with actual language instead of `'unknown'`. Backward compat: `matchesLanguage()` helper includes `'unknown'`-language phrases in queries.
- ~~**Memory retrieval agent leaks cross-language terms (EXP-054)**~~ — `MemoryRetrievalAgent.getStrugglingTermPhrases()` and `buildTeachingContext()` now filter by `query.language`. Other retrieval methods (`getRelatedTerms`, `getBridgeMemories`, `getSessionRecap`, `getEngagementPatterns`) were already correctly filtered.
- ~~**Relationship store per-avatar scoping verified (EXP-055)**~~ — `RelationshipStore` was already correctly keyed by `avatarId`. No fix needed.

### Resolved Feature Gaps (2026-04-16w — EXP-061 through EXP-065)
- ~~**Scenarios have no progression/phase awareness (EXP-061)**~~ — `ConversationDirector.preProcess()` now tracks scenario turns via WorkingMemory (`scenario_turn_{key}`) and injects phase-appropriate hints: OPENING (turns 1-2, set scene + introduce phrases), MIDDLE (turns 3-5, core practice with recast correction), WRAPPING UP (turns 6+, close scenario naturally + surface unused phrases).
- ~~**Phrase cards interrupt conversational flow (EXP-062)**~~ — Chat template in `toolPrompts.json` gains TEACHING STYLE section: inline teaching (`**phrase** (pronunciation)`) for casual conversation, full phrase card format reserved for explicit user requests or pronounce/phrase tool routing.
- ~~**Generic debriefs after scenarios (EXP-063)**~~ — `handleEndScenario()` debrief injection now requires structured 3-step format: (1) quote one correct thing the user said, (2) name one improvement with corrected form, (3) present 2 phrase cards. QUOTE requirement forces specific feedback.
- ~~**No scenario auto-suggestion from conversation (EXP-064)**~~ — `ConversationDirector` gains `detectScenarioFromMessage()` using situational cues + keywords. When detected and no scenario active, injects prompt suggesting practice mode. User decides, agent doesn't auto-start.
- ~~**Learning milestones not surfaced in conversation (EXP-065)**~~ — `checkMilestones()` enhanced with phrase count (10/25/50/100/250/500), mastery (1/5/10/25/50/100), streak, and stage change milestones. Each sets `milestone_celebration` flag in WorkingMemory consumed by next `preProcess()` with identity-framed instructions.

### Resolved Feature Gaps (2026-04-16x — EXP-066 through EXP-070)
- ~~**Avatar is reactive, has no independent life (EXP-066)**~~ — "YOUR LIFE IS HAPPENING" section added to `coreRules.json` instructing the avatar to have ongoing events independent of the user. `world_events` arrays (3-4 events each) added to all 8 avatar templates in `avatarTemplates.json`. `ConversationDirector.preProcess()` world event injection (0g) now pulls 50/50 from environmental events (`worldEvents.json`) and personal ongoing events (`avatarTemplates.json` `world_events`). Avatar shares neighborhood drama, personal dilemmas, and daily life naturally.
- ~~**No emotional vulnerability / reciprocity (EXP-067)**~~ — `vulnerability_moment` skill added to `conversationSkills.json` (trigger: warmth >= friend, 10% random chance). Wired into `ConversationDirector.preProcess()`. Warmth tiers updated: friend gets "share when you're having a bad day" instruction, close_friend gets "lean on them" instruction, family gets "mutual vulnerability" instruction. Avatar occasionally needs the user's emotional support, teaching empathy vocabulary through genuine caring.
- ~~**Sensory details are isolated facts, not immersive scenes (EXP-068)**~~ — `sensory_anchor` skill injection in `conversationSkills.json` rewritten from "mention a sensory detail" to "describe SCENES unfolding around you with micro-narratives." Grounded in Green & Brock (2000) transportation theory and Bamberg & Georgakopoulou (2008) micro-narrative research. Avatar now describes small dramas, reactions, and unfolding moments instead of static sensory facts.
- ~~**Memory callbacks feel like data recall, not genuine care (EXP-069)**~~ — Callback injection in `ConversationDirector.preProcess()` and `RelationshipStore.formatForPrompt()` rewritten from "reference this shared experience" to "show you were paying attention and you CARE." If it was a struggle, check on them. If a success, build on it. Goal is "I was thinking about you" not "I have good memory."
- ~~**No recurring bits, running jokes, or shared nicknames (EXP-070)**~~ — "DEVELOP BITS" section added to `coreRules.json` with warmth gate (acquaintance+). Acquaintance tier in `warmthLevels.json` updated with explicit bits development instruction. Seeds of recurring jokes planted through mispronunciations, repeated scenarios, and go-to phrases.

### Resolved Feature Gaps (2026-04-16z — EXP-071)
- ~~**Avatar always has the same emotional tone between sessions**~~ — `avatarMoods` section added to `systemLayers.json` with 7 moods (cheerful, tired, nostalgic, excited, restless, contemplative, playful). `ConversationDirector.preProcess()` injects a random mood on 40% of session starts via `TODAY'S MOOD:` directive. 60% neutral keeps moods occasional, not constant. Based on Berlyne (1960) moderate surprise within predictable framework.
- ~~**Avatar greets the same way regardless of relationship depth**~~ — `greetingStyle` field added to all 5 warmth tiers in `warmthLevels.json`. `ConversationDirector.preProcess()` injects warmth-appropriate greeting style on session start (`GREETING STYLE:`). Stranger=formal with translation, acquaintance=casual, friend=text-a-friend, close_friend=conversation-starter, family=no greeting at all.
- ~~**Avatar has no consistent personality anchors across sessions**~~ — `IDENTITY ANCHORS` block added to `coreRules.json`: neighborhood opinion, go-to recommendation, catchphrase/reaction. These never change, creating predictability that IS the relationship. Based on Horton & Wohl (1956) — consistency of self-presentation is the strongest predictor of parasocial bond strength.

### Resolved Feature Gaps (2026-04-16 — EXP-072 through EXP-075)
- ~~**Avatar language style doesn't evolve with relationship depth (EXP-072)**~~ — `relationshipLanguage` section added to `systemLayers.json` with 5 stages: stage_1 (formal, translate everything), stage_2 (drop translations for taught phrases, casual register), stage_3 (custom greetings, shortened forms, reference shared experiences), stage_4 (skip greetings, inside references, "between us" framing), stage_5 (unique shorthand, half-sentences, language IS intimacy). `ConversationDirector.preProcess()` maps warmth to stage (0-0.2/0.2-0.4/0.4-0.6/0.6-0.8/0.8+) and injects on every message. Separate from warmth instruction — controls HOW, not tone.
- ~~**Avatar talks about the same topics regardless of relationship depth (EXP-073)**~~ — `characterArc` section added to `systemLayers.json` with 4 stages: early (practical language, surface city knowledge), developing (opinions, stories, cultural nuances), deep (politics, philosophy, challenge views), bonded (effortless, disagree openly, talk about future). `ConversationDirector.preProcess()` maps warmth to arc (0-0.3/0.3-0.55/0.55-0.8/0.8+) and injects on every message. Arc thresholds intentionally offset from language stage thresholds so content deepening lags behind style casualization.
- ~~**Avatar doesn't develop nicknames for the user (EXP-074)**~~ — `NICKNAMES` rule added to `coreRules.json` before `DEVELOP BITS`: after 10+ exchanges at friend warmth or higher, develop a nickname based on something real (pronunciation quirk, origin, shared joke), use naturally (not every message), prefer target language, or create affectionate name variation.
- ~~**Absence/return messages are generic regardless of relationship depth (EXP-075)**~~ — `ProactiveEngine.getProactiveMessage()` gains optional `warmth` parameter. New `absenceMessage()` method implements 5-tier responses: stranger (stats-based, unchanged), acquaintance ("Oh, you're back. Been busy?"), friend ("Where have you been? I was starting to wonder."), close friend ("Finally! I have so much to tell you..."), family ("There you are." — just that). `NaviAgent.getProactiveMessage()` now passes warmth from RelationshipStore.

### Resolved Feature Gaps (2026-04-16 — Avatar Creation UI)
- ~~**Custom avatar option buried at bottom of template grid**~~ — "Create a companion" card moved to top of `AvatarSelectScreen.tsx` as primary CTA with solid highlighted styling (not dashed border). Templates demoted to "Quick start" section below. Copy updated: "Create a companion / Describe who you want by your side".
- ~~**No location/language picker in avatar creation**~~ — Location picker added to `AvatarSelectScreen.tsx` after name input. Shows cities from `dialectMap.json` grouped by language with search, "Detect my location" GPS option, and collapsible UI showing city + dialect info. Selected city's `dialectKey` flows through `buildLocationFromPreset()` to `LocationContext` passed to `onSelect()`. Start button requires location selection.
- ~~**dialectMap missing major cities**~~ — Added 8 cities to `dialectMap.json`: Barcelona (Catalan-influenced Spanish), Madrid (Castilian Spanish), Rome (Roman Italian), Bangkok (Central Thai), Berlin (Berlin German), Sao Paulo (Brazilian Portuguese), Shanghai (Shanghainese-influenced Mandarin), Buenos Aires (Rioplatense Spanish). Updated `COUNTRY_NAMES` in `locationHelpers.ts` with ES/IT/TH/DE/BR/CN/AR.

### Resolved Feature Gaps (2026-04-16 — EXP-081)
- ~~**Avatar personality is generic for cities not in dialectMap (EXP-081)**~~ — Universal location personality system added. Three new layers in `systemLayers.json`: `locationPersonality` (makes avatar a true local with opinions, favorite spots, knowledge of characters and changes), `culturalVoice` (8 language-family conversational styles: romance, germanic, east_asian, south_asian, slavic, semitic, southeast_asian, default), `locationSensory` (grounds avatar in physical sensory reality of the city). All three layers injected in `contextController.ts` `buildLocationLayer()` for EVERY city — both dialectMap cities and unknown cities. Language-family mapping (`LANGUAGE_FAMILY_MAP`) covers 50+ languages across 7 families. `AvatarSelectScreen.tsx` gains freeform city input: "Don't see your city? Type it:" with text field below preset list, supporting "City, Country" format. Custom cities create `LocationContext` with empty dialectKey/dialectInfo — universal layers still fire. Build passes, 104/104 tests pass. Barcelona dialect test: 5/6 markers (EXCELLENT).

### Resolved Feature Gaps (2026-04-16 — City & Language Picker)
- ~~**City picker limited to 15 hardcoded dialectMap cities**~~ — `AvatarSelectScreen.tsx` rewritten with new `CityPicker` component (`src/app/components/CityPicker.tsx`) backed by `cities.json` (321 deduplicated world cities). Searchable autocomplete with GPS detect, country flag emojis, debounced 300ms filtering, max 8 results displayed. Starts-with matches prioritized over contains matches.
- ~~**Language tied to city selection (no override)**~~ — New `LanguagePicker` component (`src/app/components/LanguagePicker.tsx`) + `supportedLanguages.ts` config (45 languages with native names, scripts, and country defaults). Language is fully independent of city: user in Barcelona can pick Catalan, Spanish, or any other language. Auto-suggests default language for the selected city's country but always allows override.
- ~~**Settings location section was bare text input**~~ — SettingsPanel "Location" tab (now "Location & Language") replaced with full `CityPicker` + `LanguagePicker`. Save button persists to `LocationContext`, character `target_language`, user preferences, and agent profile memory.
- ~~**`AvatarSelectScreen.onSelect` did not accept language**~~ — Callback extended to `(template, location, languageCode?)`. `App.tsx` `handleAvatarSelected` stores `target_language` on the Character object, syncs to `userPreferences.target_language`, and sets it on `agent.memory.profile`.

### Resolved Feature Gaps (2026-04-16)
- ~~**SettingsPanel Ollama models not dynamically detected**~~ — SettingsPanel Model tab previously showed Ollama as a separate card below the Apply button with a dropdown select. Refactored: Ollama is now a first-class tab alongside Cloud Free, Cloud Paid, and On-Device. Tab appears dynamically when Ollama is detected (or when current backend is `ollama`). Each installed Ollama model shown as a clickable card with name and size (fetched from `localhost:11434/api/tags`). Selection persists via `localStorage('navi_backend_pref')` + `localStorage('navi_ollama_model')`. CORS hint shown when Ollama not reachable. BackendSelectScreen also persists `navi_ollama_model` on selection for consistency.
...
### Resolved Feature Gaps (2026-07-05 — Conversation Quality + UI)
- ~~**`max_tokens` too low — responses truncated mid-sentence**~~ — Raised `chat.max_tokens` 250→400, `phrase.max_tokens` 600→700 in `toolPrompts.json`. Single highest-impact conversation quality change.
- ~~**History window too tight for OpenRouter**~~ — `ConversationScreen` now passes backend-aware history limit: 20 messages (10 turns) for OpenRouter, 8 for Ollama. Removed `chatTool`'s redundant `.slice(-8)` — double-slicing was capping history to 8 even when OpenRouter was active.
- ~~**Tier advancement counters reset on app restart**~~ — `consecutiveTargetLangMessages`, `consecutiveHelpRequests`, and `exchangesSinceTierChange` now persisted via WorkingMemory with 24h TTL (keys: `tier_consecutive_target`, `tier_consecutive_help`, `tier_exchanges_since_change`). Lazily loaded from WM on first `assessLanguageTier()` call. Counters no longer reset when director is recreated.
- ~~**Emotional state false positives (anxiety)**~~ — Added `hasPositiveCounterSignal` check: presence of positive words/emojis (cool, great, nice, awesome, haha, lol, 😊, 🎉, etc.) cancels anxiety detection. "I guess that's cool?" no longer triggers anxious state.
- ~~**Phrase card fields echoing prompt placeholders**~~ — `parseResponse()` in `responseParser.ts` now checks `phrase` and `meaning` fields via `isPlaceholderField()`. Cards with fields like "(write the actual phrase in French)" or "n/a" are emitted as plain text instead of broken phrase cards.
- ~~**Avatar repeats already-taught phrases despite instruction**~~ — `ConversationDirector.postProcess()` now stores detected phrase names in WorkingMemory (`session_taught_phrases`, 2h TTL). `chatTool` reads this list before each LLM call and appends a `DO NOT REPEAT` suffix with the last 10 taught phrases to the system message. Stateful, not just prompt-instructed.
- ~~**Scenario wrapping up too early (pure turn count)**~~ — Added `completionSignals` detection ("thank you", "got it", "ok great", "see you", etc.). WRAPPING UP phase now triggers on `currentTurn > 5` OR user signaled completion after turn 3 — whichever is first.
...
- ~~**Thinking model inline reasoning leak (qwen3:4b)**~~ — `responseParser.ts` gains `stripInlineReasoning()`: detects chain-of-thought preamble lines ("We are in...", "I need to...", etc.) and strips everything before the first human-sounding line. Applied in `ConversationScreen.tsx` at both streaming (`onToken`) and final response.
- ~~**Default Ollama model was qwen2.5:1.5b**~~ — Changed to `gemma3:4b` (follows format instructions cleanly, no reasoning leak). `gemma3:4b` preset added to `OLLAMA_PRESETS` in `ollamaProvider.ts`.
- ~~**Full Immersion Mode not implemented**~~ — Avatar now speaks ENTIRELY in `{{targetLanguage}}` using the correct local script (Devanagari for Nepali, Hangul for Korean, etc.). English only on confusion signals ("what?", "I don't understand"). Updated: `coreRules.json` LANGUAGE rule, `systemLayers.json` `languageEnforcement.template` and `identity.template`, `toolPrompts.json` `chat` and `chat_compact` templates. `chatTool.ts` now passes `targetLanguage` to both chat template interpolations.
...
- **TBLT scenario phase tracking — basic phase hints implemented, full arc configs pending** — EXP-061 added turn-based phase tracking via WorkingMemory (`scenario_turn_{key}`): OPENING (turns 1-2), MIDDLE (turns 3-5), WRAPPING UP (turns 6+) phase hints injected into preProcess. Combined with TBLT pretask (EXP-050/052) and posttask (EXP-050) skill injections. Still pending: per-scenario arc definitions in `scenarioContexts.json` (RESEARCH_ROUND5.md designs restaurant/market/directions arcs with specific turn-by-turn prompts), complication injection, and auto-debrief trigger when phase count exceeds expected turns.
...
- **Automated scorer misses non-English sensory/personality content** — Both `hasSensory` and `hasPersonality` in `liveConversationTest.ts` use English keywords only. EXP-051 confirmed: production scenarios (Vietnamese HCMC) scored 0/15 sensory and 6/15 personality automated, but manual audit shows Vietnamese sensory content ("nồi phở đang sôi" = boiling pho pot, "mùi thơm" = delicious smell) and personality ("bá đạo nhất" = most legendary, "menu paparazzi"). Same issue for Seoul Korean (EXP-041). The -0.3 gap between production (3.9) and hand-crafted (4.2) is primarily scorer limitation. Fix: add Vietnamese/Korean/Japanese/French/Nepali sensory and personality word patterns to the scorer.
...
- **Script enforcement missing from dialect-specific prompts** — EXP-088 (Osaka-ben test) revealed that dialect prompts without an explicit "ALWAYS write in Japanese script (kana/kanji)" instruction produce romanized output. The production language enforcement layer handles this, but isolated test scenarios and any prompt path that bypasses language enforcement need explicit script instructions. Affects: Japanese, Korean, Thai, Arabic, Devanagari scripts.
- **Sensory grounding bottleneck at 30%** — EXP-090 final benchmark showed sensory grounding at 30% (6/20 messages), the weakest dimension. The model prioritizes teaching content (phrase cards) over atmospheric details when both are requested. Run-to-run variance is high: the standard run scored 63% sensory while EXP-090 scored 30%. The sensory prompt IS in the system prompt but competes for attention with teaching instructions.
- **Latin script language scorer limitation** — French, Catalan, Spanish, and other Latin-script languages score low on the `hasTargetLang` metric because the scorer requires non-ASCII characters. EXP-087 (Catalan) scored 2/5 target language despite every response containing Catalan. EXP-089 (French) scored 1-2/5 despite French responses. Scorer needs a language-aware detection mode for Latin-script languages.
- **Personal context injection underutilized in Session 2** — EXP-089 showed that even when personal context (bakery experience, cafe visit) is injected into the session 2 system prompt, the model references it in only ~1/5 cases. The "reference naturally, don't announce you remember" instruction may be too subtle for smaller models. Consider stronger injection: "The user mentioned going to a bakery yesterday — bring this up when relevant."
...
- **No avatar mood system** — The avatar has no mood variation between sessions. Every session starts with the same energy. **RESEARCH_ROUND6.md** designs a `generateSessionMood()` function that produces moods (great/tired/nostalgic/excited/reflective/playful/irritated) with causes seeded from personality_details, 60% chance of no mood injection, prompt injection per mood type.
- **No relationship language stages** — The warmth system controls TONE but not the fundamental nature of communication. **RESEARCH_ROUND6.md Research Area 3** designs a 5-stage progression (standard -> shortcuts -> insider -> shorthand -> our language) that changes greetings, translation behavior, nickname emergence, and shared vocabulary over sessions 1-40+.
- **No emotional anchoring techniques** — Language taught during emotional peaks is retained 3-5x better but no system deliberately creates or leverages emotional moments for teaching. **RESEARCH_ROUND6.md Research Area 4** designs 10 specific techniques (victory, comfort, adventure, confession, laughter, nostalgia, fear-to-courage, discovery, ritual, future) with exact prompt text.
- **No world events system** — The avatar's world is static between sessions. Nothing "happens" around them. **RESEARCH_ROUND6.md Research Area 2** designs 20 world events across 4 categories (street, people, sensory, cultural) with callback seeds for future sessions. Events trigger at 30% per session, max 1 per session.
- **No narrative arc system** — The avatar has no ongoing storylines in their life. **RESEARCH_ROUND6.md** designs multi-session narrative arcs (5-10 sessions each) connected to personality_details fields, with beginning/middle/end beats and embedded language teaching moments.
- **No avatar agency / initiative patterns** — The avatar responds but rarely initiates unprompted topic changes, pushes back on boring topics, or brings up their own interests. **RESEARCH_ROUND6.md Mechanism 5** designs prompt patterns for perceived agency.
- **No core identity anchors for character consistency** — The avatar's opinions and personality traits can drift over long conversations. **RESEARCH_ROUND6.md Mechanism 1** designs fixed identity anchors (opinions that never change, recurring character references, sensory environment constants) that enforce personality consistency across hundreds of sessions.
- **No reciprocal vulnerability detection** — When the user shares something personal, no system detects the disclosure depth and triggers the avatar to match it. **RESEARCH_ROUND6.md Mechanism 4** designs vulnerability matching with depth detection and reciprocal disclosure prompts.
- **No city knowledge tier system** — `contextController.buildLocationLayer()` treats all cities identically. Cities not in `dialectMap.json` get a generic "speak the local language of X" fallback with no cultural scaffolding. **RESEARCH_ROUND7.md Area 1** designs a 4-tier system (A=global megacities, B=regional capitals, C=smaller cities, D=small towns) with tier-appropriate prompt strategies, auto-detection via population + dialectMap presence, and regional anchoring for Tier D. The "Barcelona problem" (3.7 vs 4.9 for Seoul) is diagnosed as a personality gap, not a dialect gap.
- **No dialect bridge system** — When a user who learned Spanish in Barcelona travels to Buenos Aires, the system starts from scratch. No vocabulary transfer, no pronunciation shift teaching, no dialect variant linking. **RESEARCH_ROUND7.md Area 2** designs a `DialectBridgeContext` type, `DIALECT_VARIANT_OF` edge type for KnowledgeGraph, SR starting-level bonuses for shared concepts, and a bridge prompt template for the first 3-5 messages in a new dialect region.
- **No real-user testing infrastructure** — All quality metrics are automated rubric scores on 5-message synthetic conversations. No session length tracking, no return rate measurement, no retention testing (24h/7d/30d phrase recall), no real-world use reporting. **RESEARCH_ROUND7.md Area 3** designs a 30-day testing protocol with 20 users across 4 cohorts, behavioral signals, weekly surveys, exit interviews, and success criteria.
- **Hindi language not supported** — 600M+ speakers, massive diaspora. Devanagari pipeline already exists (Nepali). Needs dialectMap entries for Delhi and Mumbai. **RESEARCH_ROUND7.md Area 4** rates this P1 (low effort, high market).
- **Indonesian language not supported** — 270M+ speakers (with Malay). Latin script, straightforward phonology. Needs dialectMap entries for Jakarta and Bali. **RESEARCH_ROUND7.md Area 4** rates this P1 (low-medium effort, huge market).
- **Turkish language not supported** — 80M+ speakers. Istanbul is a Tier A city. Agglutinative morphology needs teaching strategy adjustment beyond phrase cards. **RESEARCH_ROUND7.md Area 4** rates this P2.
- **Tagalog/Filipino language not supported** — 120M+ speakers. Taglish code-switching is a natural fit for NAVI's code-switching framework. **RESEARCH_ROUND7.md Area 4** rates this P2.
- **Russian language not supported** — 250M+ speakers. Cyrillic romanization pipeline needed. Grammar complexity (6 cases, verbal aspect) requires teaching scaffolding beyond phrases. **RESEARCH_ROUND7.md Area 4** rates this P2.
- **Swahili language not supported** — 100M+ speakers. Underserved market. LLM output quality uncertain (smaller training corpus). **RESEARCH_ROUND7.md Area 4** rates this P3.
