...
**Last updated: 2026-04-16k** (Experiments EXP-021 through EXP-025 — deep teaching mechanics: EXP-021 — negotiation of meaning: CONFUSION OVERRIDE in `coreRules.json` rewritten from immediate native-language switch to two-step process (assess willingness -> try ONE simpler rephrase in target language -> switch to native only on shutdown signals or failed rephrase); `negotiation_of_meaning` skill added to `conversationSkills.json` (Long 1996 Interaction Hypothesis); EXP-022 — language play and humor: `language_play` skill added to `conversationSkills.json` (trigger: user_comfortable, tier 2+; Cook 2000, Bell 2005); `languagePlay` section added to `learningProtocols.json` with 9 tongue twisters (3 Japanese, 3 French, 3 Korean) as structured data with text/romanization/meaning/difficulty/sound_focus; EXP-023 — input flooding: `input_flooding` skill added to `conversationSkills.json` (use target structure 3-4 times in varied contexts without teaching it; Schmidt 1990 Noticing Hypothesis, Wong 2005); EXP-024 — register awareness: audited existing formality infrastructure (scenarioContexts.json has formality_adjustment on all 20 scenarios, avatarTemplates.json has default_formality, dialectMap.json has formality_default); `register_awareness` skill added to `conversationSkills.json` (Bardovi-Harlig 2001, Kasper & Rose 2002); REGISTER SWITCHING instruction added to `conversational` learning stage in `systemLayers.json` (show casual+formal versions side by side); EXP-025 — productive failure: `productive_failure` skill added to `conversationSkills.json` (trigger: user_at_functional_or_higher; create word gap for ONE exchange then give answer with enthusiasm; Bjork 1994, Kapur 2008); `advanced_technique` block added to `elicitation` protocol in `learningProtocols.json` distinguishing productive failure (acquisitive) from elicitation (corrective); build passes, 104/104 tests pass)
