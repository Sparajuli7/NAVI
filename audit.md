...
**Last updated: 2026-04-16p** (Experiments EXP-036 through EXP-040 -- sensory grounding, Kathmandu language, backstory seeds, compact rules, multi-turn coherence: EXP-036 -- sensory grounding strengthened in `coreRules.json` ("NOT OPTIONAL", 2-3 message cadence, MUST rule); per-scenario sensory palettes added to test prompts (Tokyo/Paris/Kathmandu/Seoul); sensory scorer expanded with 20+ scenario-specific words; Paris sensory 2/5 -> 5/5; overall sensory 50% -> 55%; key insight: scorer is English-biased, misses Korean/Japanese atmospheric text; EXP-037 -- FRUSTRATION vs CONFUSION distinction added to `coreRules.json` (frustration about language = stay in target language with native glosses; shutdown = switch to native); Kathmandu test prompt rewritten with explicit Nepali requirement + character backstory; **Kathmandu target language 40% -> 100%** (biggest single-dimension improvement); EXP-038 -- BACKSTORY SEEDS rule 5 added to both templates in `characterGen.json` (specific opinion, funny anecdote, pet peeve, sensory detail); based on EXP-034 finding that specific details >> generic instructions; EXP-039 -- compact ~460-token core rules tested on qwen2.5:1.5b; **3.1 -> 3.8 (+23% improvement)**; open loops 35% -> 100%, sensory 10% -> 40%; target language regressed 85% -> 40% (needs stronger LANGUAGE line); confirms RESEARCH_ROUND3 finding: examples > instructions for small models; EXP-040 -- 12-turn extended conversation with gemma4:e2b; CRITICAL: sensory grounding 0/12, open loops collapse after turn 6 (33% -> 0%), personality degrades at turn 12; sycophancy remains 100% (NEVER rules resilient); target language 100% (identity layer stable); validates SESSION PACING (EXP-030) -- wrap at 8-10 is correct; build passes, 104/104 tests pass)
