...
> Note: this file is a historical audit trail. Going forward, record cleanup detail in git commit messages — not as new dated entries here. Track open work in CLAUDE.md "Known Gaps".

**Last updated: 2026-06-28** (8-pass cleanup + typecheck infrastructure. Added `tsconfig.json` (strict) + `@types/react`/`react-dom`/`node` + `pnpm run typecheck` — TS was never type-verified before (Vite/esbuild only). Surfaced and fixed 35 pre-existing type errors → **0 errors**. Pass 3 (dead code): deleted `utils/avatarPrefs.ts`, empty `postcss.config.mjs`, and dead exports across tts/stt/storage/generateAvatarImage/pronunciationLookup/locationHelpers/useNaviAgent/knowledgeGraph/CityPicker. Pass 7 (legacy): removed unused NewChatBubble component, made `OCRResult.confidence` required. Pass 2 (types): unified `DialectMap` (was 3x) into `types/config.ts`, fixed `ModelStatus` import/reexport. Pass 4 (circular deps): madge confirms 0 cycles. Pass 1 (DRY): new `agent/tools/toolHelpers.ts` (getToolConfig + getUserNativeLanguage, collapsed 4x+7x), `utils/formatBytes.ts` (3x), BackendSelectScreen reuses `listOllamaModels()`. Pass 5 (weak types): new `types/speech.d.ts` (Web Speech API), `ParsedSegment` → discriminated union, fixed real `imageUnderstanding.ts` timing bug (ocrMs/classificationMs undeclared), added `isProactive` to MessageMetadata. Pass 6 (try/catch): removed ~10 catches around statically-bundled config (can't throw), improved ~14 silent boundary catches to log with context; kept genuine I/O/network/provider-rotation handling. Pass 8 (slop): removed remaining debug logs + stale TODOs. CLAUDE.md condensed 631→~155 lines (dropped dated changelog, now in git). Build ok, 104/104 tests, 0 type errors, 0 cycles.)

**Previously: 2026-04-18b** (Major codebase cleanup — -9,347 net lines removed across 104 files. Dead code: deleted 48 unused shadcn/ui components, 5 dead app components (AIAvatarDisplay, AvatarBuilder, ActionCard, ContextualCard, figma/ImageWithFallback), dead files (services/ocr.ts, services/modelManager.ts, agent/director/types.ts, utils/contextManager.ts). Dependencies: removed 54 unused npm packages. DRY: consolidated countryFlag() from 5 copies to 1 shared util, COUNTRY_NAMES from 2 divergent maps to 1 (46 entries), AGE_GEN_MAP from 3 copies to 1, registerAllTools deps from 4 inline copies to buildToolDeps() method, localStorage check from 3 copies to class property. Types: extracted UserMode (was inline 15x) and FilterMode (3x) to core/types.ts, ConversationGoal moved to core/types.ts. Circular deps: broke both cycles (character↔avatarPrefs, director↔SessionPlanner) — madge reports 0 cycles. Comments: stripped 60+ EXP-XXX experiment tracking comments, removed ~90 console.log debug traces, trimmed verbose module headers. Error handling: replaced 6 silent .catch(()=>{}) with .catch(console.warn). Infrastructure: extracted isWebGPUSupported to utils/platform.ts. Build passes, 104/104 tests pass.)

**Previously: 2026-04-18** (Circular dependency fixes + error visibility + type consolidation)

**Previously: 2026-04-18** (Type consolidation: `UserMode` type extracted from 18 inline `'learn' | 'guide' | 'friend' | null` occurrences across 9 files into single canonical export in `agent/core/types.ts`. `FilterMode` type extracted from local definitions in FlashcardDeck.tsx and KnowledgeGraphExplorer.tsx into same file. All consumers now import from core/types. Build passes.)

**Previously: 2026-04-17** (EXP-112 — Praktika-inspired language approach rewrite: Complete overhaul of language-related system prompt instructions. Root cause of repetition loops and wrong-language responses: contradictory instructions — locationLayer said "SPEAK IN FRENCH" while languageEnforcement said "speak mostly English." Fix inspired by Praktika AI research: beginner chats should be 80-90% user's language with target phrases embedded in bold with pronunciation. Changes: (1) `coreRules.json` rewritten from ~2,100 to ~480 tokens; "NEVER REPEAT YOURSELF" now rule #1; language rule simple and unambiguous; fewShotExamples aligned to new approach. (2) `systemLayers.json`: identity.template shortened; languageCalibration tiers flipped (tier_1=80-90% user language, not "lead in local language"); languageEnforcement simplified; modeInstructions.learn changed from "full immersion" to "learning mode with calibration"; codeSwitchingPriority shortened; emotionalMirroring, conversationNaturalness, locationPersonality, locationSensory all compressed 60-80%; gauging_question, session_opener, free_conversation aligned. (3) `toolPrompts.json` chat.template rewritten as 5 numbered rules (~150 tokens vs ~800). (4) `contextController.ts` locationLayer no longer emits "SPEAK IN [LANG]" — now "talk mostly in user's language, embed target phrases"; internal monologue L14 removed. Total prompt reduction: ~65%. All language instructions now unified. Build passes.)
