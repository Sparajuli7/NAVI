...
**Last updated: 2026-04-16ag** (EXP-091 through EXP-095: EXP-091 — Paris open loop variance diagnosed; personality-amplification guidance added to `conversationSkills.json` open_loop skill and `toolPrompts.json` chat template (sarcastic characters tease, stern characters challenge, warm characters wonder — hooks should feel like the character). EXP-092 — Rome dialect test created (`romeDialectTest.ts`): Marco the trattoria owner in Trastevere, 3-message flow, 6-marker scoring. EXP-093 — Berlin dialect test created (`berlinDialectTest.ts`): Kai the DJ in Kreuzberg, 3-message flow, 6-marker scoring. EXP-094 — Token budget audit confirmed Core Rules (4140t) was being silently dropped pre-EXP-096; post-EXP-096 budget is marginally tight (~3422 for MUST+HIGH, 350 over 3072). EXP-095 — BUG FIXED: Ollama backend not restored on restart — constructor only checked `'webllm'|'openrouter'`, missing `'ollama'`; also added Ollama model name persistence via `navi_ollama_model` localStorage key. Build passes, 104/104 tests pass.)
