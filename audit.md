...
**Last updated: 2026-07-05** (WebLLM → OpenRouter migration: Removed `@mlc-ai/web-llm` from package.json and deleted `src/agent/models/llmProvider.ts` (the sole WebLLM importer). `openRouterProvider.ts` rewritten: endpoint changed to `/api/chat` local proxy, `Authorization` header removed (key is now server-side only in `OPENROUTER_API_KEY`), constructor takes `models?: string[]` only (no API keys), `setApiKeys()` is a no-op for backward compat, model rotation kept (primary=qwen/qwen3-32b, fallback=meta-llama/llama-3.3-70b-instruct). New `api/chat.ts` Vercel serverless function: reads `OPENROUTER_API_KEY` from server env, forwards POST to OpenRouter, streams SSE. `vercel.json`: SPA rewrite now excludes `/api/*`. `agent/index.ts`: removed webllm branch, openRouterKeys env parsing, and `LLM_PRESETS`/`LLMProvider` exports; 'auto'→openrouter; context budget raised to 6000 for cloud. `App.tsx`: removed `no_webgpu` AppPhase and WebGPU check; first launch defaults to 'openrouter'. `BackendSelectScreen`: removed "On-Device" card and all webllm handling. `SettingsPanel`: removed webllm card, `LLM_PRESETS` import, `pendingWebllmPreset` state. `ModelDownloadScreen`: copy updated to cloud terminology. `useNaviAgent.ts`: removed `webllmPreset` state; `switchBackend` sig simplified to `'openrouter'` only. `generateAvatarImage.ts`: Step A now calls `/api/chat` proxy instead of direct OpenRouter with client key. `.env.example`: changed from `VITE_OPENROUTER_API_KEY=` to `OPENROUTER_API_KEY=`. `openRouterProvider.test.ts` rewritten to match new keyless API (removed Authorization header test, added proxy-URL test, `setApiKeys()` no-op test, updated constructor calls). 105/105 tests pass.)
