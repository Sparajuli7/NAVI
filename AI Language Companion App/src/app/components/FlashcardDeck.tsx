...
import {
  CheckCircle, XCircle, Clock, Volume2, Layers, X, Zap, Trophy, LayoutList,
} from 'lucide-react';
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            <span
              className={`text-[10px] font-medium px-2 py-0.5 rounded-full border uppercase tracking-wide ${MASTERY_COLORS[phrase.mastery]}`}
            >
              {phrase.mastery}
            </span>
        ...
        >
          ...
          {phrase.pronunciation && (
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-[10px] text-white/40 mb-0.5 uppercase tracking-wide">Say</p>
          ...
          {phrase.meaning && (
            <div>
              <p className="text-[10px] text-white/40 mb-0.5 uppercase tracking-wide">Meaning</p>
...
export function FlashcardDeck({
  ...
  const FILTERS: { key: FilterMode; label: string; Icon: typeof LayoutList }[] = [
    { key: 'all', label: 'All', Icon: LayoutList },
    { key: 'struggling', label: 'Hard', Icon: Zap },
    { key: 'due', label: 'Due', Icon: Clock },
    { key: 'mastered', label: 'Done', Icon: Trophy },
  ];
    ...
    <div className="flex flex-col h-full bg-[#0A0A0F] text-white">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="font-serif text-lg text-[#F5F0EB]">Deck</h2>
            <p className="text-[11px] text-white/40">{phrases.length} phrases</p>
          </div>
        </div>
        {onClose && (
          <button
        ...
        <AnimatePresence mode="popLayout">
          {filtered.length === 0 ? (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="flex flex-col items-center justify-center py-20 gap-3 text-white/30"
            >
              <Layers className="w-9 h-9" strokeWidth={1.25} />
              <p className="text-sm">
                {phrases.length === 0 ? 'Chat to collect phrases' : 'Nothing here'}
              </p>
            </motion.div>
          ) : (
